...
if (isset($_POST['alogin'])) {
  ...
  if(mysqli_num_rows($run1)>=1 && $name!=""){
    $sql2 = "SELECT * FROM admin WHERE name='$name' AND psw='$psw'";
    $run2 = mysqli_query($con, $sql2);
    if(mysqli_num_rows($run2)==1 && $psw!=""){
      $_SESSION['aname'] = $name;
      header('location:../admin.php');
    }
    else{
    ...
    echo '<script>alert("Admin doesn\'t exist, Please Enter valid Admin name.!!")</script>';
  }
}
...
if (isset($_POST['login'])) {
  ...
  // $flag = 0;
  $sql1="SELECT * FROM user_info WHERE uname='$name'";
  $run1=mysqli_query($con,$sql1);
  $row1=mysqli_fetch_array($run1);
  if (mysqli_num_rows($run1)>=1 && $name!="") {
      $sql2="SELECT * FROM user_info WHERE uname='$name' AND psw='$psw'";
      $run2=mysqli_query($con,$sql2);
    ...
    // $redrct = header('location:signup.php');
    // echo '<script type="text/JavaScript">';
    // echo 'if(!alert("User doesn\'t exist..!!")){';
    // echo '$redrct';
    // echo '}</script>';
    ...
    echo '<script>alert("User doesn\'t exist, Please sign up first..!!")</script>';
  }
...
if (isset($_POST['booking'])) {
	...
	$date=date("Y-m-d");
	$vacant=$seat-$no_p;
  if($vacant>=0){ // Check if vacant seat is not becoming negative.
    ...
    echo "Sorry. Your requested amount of seats are not available..!!";
    $vacant = $seat + $no_p;
  }
}
